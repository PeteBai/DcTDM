...
if not do_short:
    ...
    for dataset in os.listdir(os.path.join(".","videos")):
        ...
        for vid in vids:
            depth_ct = int(vid.split(".")[0])
            new_depth_name = str(depth_ct).zfill(depth_zero_ct) + ".npy"
            the_line = "{} {} {} {} {}\n".format(os.path.join("videos",dataset,vid),os.path.join("depth",dataset),new_depth_name,15,"A vehicle driving in-road, kitti_02 style.")
            if dataset == "kitti_02":
                f.write(the_line)  
f.close()
