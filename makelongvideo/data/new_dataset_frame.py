...
class MakeDepthVideoDataset(Dataset):
    ...
    def form_video_seq(self,video_path,parent_folder,start_file,vid_len):
        start_ct = int(start_file.split(".")[0])
        filename_len = len(start_file.split(".")[0])
        vid_batch = []
        for i in range(vid_len):
            frame_to_open = str(start_ct+i).zfill(filename_len) + ".png"
            frame = os.path.join(video_path,parent_folder,frame_to_open)
            vid_batch.append(frame)
        return vid_batch

    def form_video(self, vid_seq):
        vid_batch = []
        for item in vid_seq:
            frame = Image.open(item).convert('RGB')
            vid_batch.append(frame)
        return vid_batch

    def find_elements(self, lst1, idx_list):
        return [lst1[i] for i in idx_list]
    ...
    def getvr(self, index, sample_frame_rate):
        ...
        for i in range(depth_loop_ct):
            ...
            one_depth = np.load(os.path.join(video_path,info[1],name_to_open))
            one_canny = Image.open(os.path.join(video_path,info[3],canny_to_open)).convert('L')
        ...
        try:
            ...
            dataset_name = info[1].split("/")[-1]
            vr = self.form_video_seq(video_path,"images/"+dataset_name,info[4],depth_loop_ct)
    ...
    def __getitem__(self, index):
        ...
        video = self.find_elements(vr,sample_index)
        video = self.form_video(video)
